...
## LLM Exercise — Chapter 00: Claude Code Basics (Text Adventure Project)

**Project:** Build a Text Adventure or Console Game — across the semester, build a real playable text-based game in Python. Each chapter adds capabilities. By Ch 15: a complete game with classes, file save/load, and a playtest-data analysis.
**What you're building this chapter:** the project's foundation — pick the game premise, set up the project folder, write the game-vision document.
**Tool:** **Claude Code** as primary throughout. **Claude Project** "Text Adventure — [Game Name]" for design conversations.
...
```
I'm starting a semester-long project to build a text-based
adventure game in Python. Each chapter of an Intro Python book
adds capabilities. By Ch 15: a complete playable game with classes,
save/load, and analysis of playtest data.

Help me set up. Ask me ONE question at a time, waiting for my
answer.

1. The genre and setting. Pick ONE:
   - Fantasy adventure (dungeons, swords, magic).
   - Sci-fi space exploration (ships, planets, aliens).
   - Mystery / detective (investigating a crime).
   - Horror (escape a haunted house).
   - Historical (a specific era — Victorian London, ancient
     Rome).
   - Slice-of-life (mundane but oddly compelling — a barista
     simulator).
   - Custom: describe in one sentence.

2. The premise. One paragraph: who is the player? What's their
   goal? What's the central conflict?

3. The vibe. Comedic? Earnest? Dark? Whimsical? Pick one. The
   tone shapes every line of text the player will read.

4. The scope. A short jam-style game (10-20 rooms, 1-2 hours of
   playtime) is realistic for one semester. State whether you
   want short-and-finished or larger-but-incomplete.

5. The project folder. Set up:
   /text-adventure/
     game.py            # main entry point (placeholder for now)
     README.md          # this vision document goes here
     /data/             # world data, save files (later chapters)
     /tests/            # test files for verification

After all five answers, do TWO things:

A) Write a 400-600 word **Vision Document** (`README.md`):
   - Game title and one-line tagline.
   - The premise (audience, player goal, central conflict).
   - The vibe and three example lines of in-game text that
     establish it.
   - The 5-7 game mechanics you want (movement between rooms,
     inventory, combat, NPCs, save/load, etc.).
   - Out-of-scope: what you're NOT building (multiplayer, graphics,
     network play, etc.).
   - The acceptance test for Ch 15: a single specific scenario
     the finished game must support end-to-end (player walks from
     room A, picks up item X, fights enemy Y, saves the game,
     loads it back, completes the goal).

B) Write a 100-line `game.py` skeleton — placeholder functions
   for the features the vision document describes, all empty
   bodies for now. Use Claude Code to verify it runs without
   errors (`python game.py` should produce the game-title banner
   and exit cleanly).

End with: state the verification commitment you'll hold across
all 16 chapters — "I will not commit code I haven't tested
myself, even if Claude says it works." This is the discipline
this book is teaching.
```

---

**What this produces:** A Vision Document + an empty-but-runnable `game.py` skeleton. The acceptance-test scenario in the vision doc is what Ch 15 will validate.

**How to adapt this prompt:**

- *For your own project:* The vision document is what keeps the project on track. Most students who fail this project drift because they didn't commit to scope. The "out of scope" list is the discipline.
- *For ChatGPT / Gemini:* Works as written.
- *For Claude Code:* Essential. The setup is literally creating files; Claude Code does that natively.
- *For a Claude Project:* The Vision Document goes in the project-level instructions so every later chapter's prompt knows the game premise.

**Connection to previous chapters:** This is the project's opening.

**Preview of next chapter:** Chapter 1 covers Python statements — variables, input, output, arithmetic. Your first runnable game code: the title banner + "What's your name, brave adventurer?" prompt + a personalized welcome.
